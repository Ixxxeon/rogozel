...
export class MicroserviceComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.microserviceService
      ...
      .subscribe((res: HttpResponse<IMicroservice[]>) => this.paginateMicroservices(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IMicroservice) {
    return item.id;
  }

  registerChangeInMicroservices() {
    this.eventSubscriber = this.eventManager.subscribe('microserviceListModification', () => this.loadAll());
  }

  delete(microservice: IMicroservice) {
  ...
  protected paginateMicroservices(data: IMicroservice[], headers: HttpHeaders) {
